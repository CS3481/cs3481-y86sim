...
{
   if (srclow < 0 || dstlow < 0 || ((dstlow + (length-1)) > 63) || ((srclow + (length-1)) > 63))
   {
       return dest;
   }
   ...
   else
   {
       ...
       uint64_t dClear = clearBits(dest, dstlow, (dstlow + (length-1)));
       
       sBits = sBits << dstlow;
       uint64_t result = dClear | sBits;
       return result;
   }
}
...
bool Tools::addOverflow(uint64_t op1, uint64_t op2)
{
   ...
   return ((sign(sum) != sign(op1)) && (sign(sum) != sign(op2)));
}
...
bool Tools::subOverflow(uint64_t op1, uint64_t op2)
{
   ...
   return ((sign(op2) != sign(op1)) && (sign(diff) == sign(op1)));
}
